...
EAP.homeContent = {
  ...
  james: {
    ...
    focus: {
      ...
      items: [
        { severity:'critical', source:'teams',   title:'Vikram @-mentioned you about auth token defect',  desc:"\"Auth API still blocking us · what's ETA on session expiry fix?\" · 14m ago",  tab:'board', stat:{ kind:'pill', value:'@you', tone:'crit' } },
        { severity:'critical', source:'product', title:'Push Auth Session Expiry to review',            desc:"You're blocking Payments' auth token fix — open 2 days",                         tab:'board', stat:{ kind:'days', value:2, label:'days' } },
        { severity:'high',     source:'product', title:'Close out review queue before Friday',          desc:'Validation Rules at 80% — needs sign-off',                                       tab:'board', stat:{ kind:'pct', value:80, of:100 } },
        { severity:'high',     source:'ai',      title:'AI drafted 3 stories from Auth retro notes',    desc:'Migrate auth tests · Adaptive session · Step-up auth · ready for your review',   tab:'board', otto:'Otto drafted 3 stories from your retro notes · Migrate auth tests · Adaptive session · Step-up auth — review and add acceptance criteria', stat:{ kind:'pill', value:'AI', tone:'ai' } },
        { severity:'medium',   source:'product', title:'New story added Day 5 — re-balance',           desc:'Biometric Onboarding pulled in mid-cycle',                                        tab:'board', stat:{ kind:'count', value:5, label:'pts added' } },
    ...
    state: {
      ...
      tiles: [
        { kind:'progressring', label:'My sprint pts',    pct: 75, valueLabel:'18 / 24 pts', tab:'board' },
        { kind:'dotstrip',     label:'Sprint clock',     total:10, current:7, dotColors:['done','done','done','done','active','active','active','pending','pending','pending'], tab:'board' },
        { kind:'sparkline',    label:'Personal velocity (last 4 sprints)', data:[16,18,20,22], unit:'pts', trendDelta:'+2', trendDir:'up', tab:'board' },
        { kind:'numpair',      label:'Auth Team flow', left:{ value:'4', label:'In flight' }, right:{ value:'0', label:'Defects' }, tab:'board' }
      ]
    },
    next: {
  ...
  dev: {
    ...
    focus: {
      ...
      items: [
        { severity:'critical', source:'product', title:'Auth token defect · ownerless 4 days',               desc:'Blocked since Day 2 — escalation window closing',                              tab:'board', stat:{ kind:'days', value:4, label:'days' } },
        { severity:'critical', source:'teams',   title:'Vikram in #payments-team · "Auth token still blocked"', desc:'AI drafted reply with current status + ETA — review and send',              tab:'board', stat:{ kind:'pill', value:'@team', tone:'crit' } },
        { severity:'high',     source:'product', title:'3 stories at risk for Sprint 2 close',               desc:'Biometric login, Auth token and Payment build below burn pace',                tab:'board', stat:{ kind:'count', value:3 } },
        { severity:'high',     source:'calendar',title:'Sprint retro Mon · agenda not drafted',          desc:'AI draft ready · pulls top 3 themes from last 5 standups',                       tab:'board', otto:'Otto drafted your retro agenda from the top 3 themes across 5 standups — Auth API blocker · story scoping · late refinement — review before circulating', stat:{ kind:'pill', value:'AI', tone:'ai' } },
    ...
    state: {
      ...
      tiles: [
        ...
        { kind:'numpair',   label:'Pts done · committed', left:{ value:'8', label:'Done' }, right:{ value:'46', label:'Committed' }, tab:'board' }
      ]
    },
    next: {
      title: 'Sprint 2 closes Friday',
...
EAP.activityFeed = {
  ...
  james: [
    { time:'3m ago',  source:'ai',       title:'Closed PR #4291 on Validation Rules',         meta:'Validation rules merged · story advanced to In Review',          tab:'board', otto:'Otto auto-merged PR #4291 · Validation Rules is now In Review — check story status' },
    { time:'12m ago', source:'ai',       title:'Drafted regression tests for Biometric Re-auth', meta:'8 cases generated from Validation Rules acceptance criteria',   tab:'board', otto:'8 regression test cases generated from Validation Rules acceptance criteria — review before running' },
    { time:'42m ago', source:'product',  title:'Linked branch to Auth Session Expiry story',   meta:'Auto-status: In Progress',                                       tab:'board', otto:'Otto auto-linked your branch to Auth Session Expiry and updated status to In Progress' },
    { time:'1h ago',  source:'teams',    title:'Vikram @-mentioned you',                        meta:'#payments-team · "Auth API still blocking us"',                  tab:'board', otto:'Vikram flagged Auth API dependency · Payments team still blocked on auth token defect — Auth Session Expiry is the blocker' },
  ...
  dev: [
    { time:'3m ago',  source:'ai',       title:'Updated impediment log',                      meta:'Auth token defect ownerless 4 days · escalation drafted',       tab:'board', otto:'Otto drafted an escalation for auth token defect — ownerless for 4 days · Auth API dependency still unresolved' },
    { time:'12m ago', source:'ai',       title:'Drafted retro agenda from 5 standups',        meta:'Top 3 themes: Auth API, story scoping, late refinement',         tab:'board', otto:'Retro agenda drafted from 5 standups · top themes: Auth API blocker · story scoping · late refinement' },
    { time:'28m ago', source:'teams',    title:'Vikram in #payments-team',                    meta:'"Auth token defect still blocked, what\'s next?" · reply drafted', tab:'board', otto:'Otto drafted a reply to Vikram about auth token defect status — review before sending' },
    { time:'1h ago',  source:'product',  title:'Flagged Biometric Fallback Flow not started', meta:'Day 4 · Sprint 3 carry-over risk',                               tab:'board', otto:'Biometric Fallback Flow not started by Day 4 — Otto flagged as Sprint 3 carry-over risk' },
    { time:'2h ago',  source:'calendar', title:'Sprint retro Mon 11am · attendees confirmed', meta:'7 of 8 RSVP\'d',                                                tab:'board', otto:'Sprint retro confirmed for Monday 11am · 7 of 8 team members RSVP\'d' }
  ],
  priya: [
    ...
    chains: [
      {
        ...
        nodes: [
          { text: 'Payments team blocked · 2 days', kind:'blocked', tab:'board' },
          { text: 'Auth API integration waiting',   kind:'blocked', tab:'board' },
          { text: 'Payment Confirmation at 20%',    kind:'risk',    tab:'board' }
        ],
        action: { label:'Start review now', tab:'board' }
      },
      {
        trigger: 'Biometric Onboarding added mid-cycle · Day 5',
        nodes: [
          { text: '5 pts added · burn pace affected', kind:'warn', tab:'board' },
          { text: '3 items below close pace',         kind:'warn', tab:'board' }
        ],
        action: { label:'View sprint', tab:'board' }
      }
    ]
  },
  riya: {
    ...
    chains: [
      {
        ...
        nodes: [
          { text: 'Auth integration blocked',    kind:'blocked', tab:'board' },
          { text: 'Payment screen waiting',      kind:'blocked', tab:'board' },
          { text: 'Payment Confirmation at 20%', kind:'risk',    tab:'board' }
        ],
        action: { label:'Escalate to Auth', tab:'board' }
      },
      {
        trigger: '3 stories below burn pace',
        nodes: [
          { text: 'Sprint 2 forecast · 64%',           kind:'warn', tab:'board' },
          { text: 'System demo line-up at risk · Fri',  kind:'warn', tab:'board' }
        ],
        action: { label:'View sprint', tab:'board' }
      }
    ]
