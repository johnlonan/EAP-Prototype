...
EAP.renderTimeline = function() {
  ...
  // Header: Calendar → PI → Sprint (inner div scales to zoom width for scroll sync)
  h += '<div class="tl-hdr"><div class="tl-hdr-left" style="width:'+leftW+'px;"></div><div class="tl-hdr-right" style="overflow:hidden;">';
  h += '<div id="tl-hdr-inner" style="width:'+zoomW+';display:flex;flex-direction:column;">';
  h += tlCalBand(range, td);
  h += tlPiBand(range, td);
  if (showSp) h += tlSpBand(range, td);
  h += '</div></div></div>';

  // Milestones (inner div scales to zoom width for scroll sync)
  ...
  h += '<div class="tl-ms-area" style="height:' + Math.max(28, ms.height + 8) + 'px;"><div class="tl-ms-left" style="width:'+leftW+'px;"><span class="tl-ms-title">' + flagSvg + ' Milestones</span></div>';
  h += '<div class="tl-ms-right"><div id="tl-ms-inner" style="width:'+zoomW+';height:100%;position:relative;">' + ms.html + '</div></div></div>';
  ...
  h += '<div class="tl-zoom-group">';
  [1, 2, 3].forEach(function(z) {
    h += '<button class="tl-zoom-btn' + (zoom === z ? ' active' : '') + '" data-tl-zoom="' + z + '">' + z + '×</button>';
  });
  h += '</div>';
...
EAP.showDepPopover = function(x, y, dep) {
  ...
  function findItem(id) {
    ...
    ['Story','Defect','Incident','Problem'].forEach(function(k) { all = all.concat(bl[k] || []); });
