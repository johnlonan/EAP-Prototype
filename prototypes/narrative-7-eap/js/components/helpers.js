...
EAP.isWorkItemType = function(type) {
  return type === 'Story' || type === 'Defect' || type === 'Incident' || type === 'Problem';
};

// ── Column body style — shared by Board and Track tabs ─
EAP._colBodyStyle = 'background:#F8F9FA;border:1px solid #E3E2DF;border-top:none;border-radius:0 0 12px 12px;padding:8px;min-height:100px;flex:1;box-shadow:0 4px 6px -1px rgba(0,0,0,0.10),0 2px 4px -1px rgba(0,0,0,0.06);';

// ── Dependency check — either direction (from or to) ───
EAP.hasDependency = function(id) {
  if (!id) return false;
  var deps = (EAP.featureDeps || []).concat(EAP.wiDeps || []);
  for (var i = 0; i < deps.length; i++) {
    if (deps[i].from === id || deps[i].to === id) return true;
  }
  return false;
};
...
EAP._ADD = EAP.icon ? EAP.icon('plus', 12, 'style="stroke:#343331;color:#343331"') : '';
...
EAP._typeColors = { goal:'#4d4c4a', epic:'#6d28d9', capability:'#0369a1', feature:'#2563EB', story:'#8B5CF6', defect:'#DC2626', incident:'#D97706', problem:'#C2410C',
  Goal:'#4d4c4a', Epic:'#6d28d9', Capability:'#0369a1', Feature:'#2563EB', Story:'#8B5CF6', Defect:'#DC2626', Incident:'#D97706', Problem:'#C2410C', 'Work Item':'#8B5CF6' };
...
EAP.typeCell = function(type) {
  ...
  var iconKey = key === 'incident' ? 'alert-circle' : key === 'problem' ? 'layers' : key;
...
EAP.subtlePill = function(st) {
  var m = {
    ...
    Implementation: 'rgba(154,52,18,0.08);color:#9a3412', Blocked: 'rgba(220,38,38,0.08);color:var(--color-error)',
...
EAP.pgWire = function(scope) {
  var root = scope || document;
  root.querySelectorAll('[data-pg-select]').forEach(function(sel) {
  ...
  root.querySelectorAll('[data-pg-prev]').forEach(function(btn) {
  ...
  root.querySelectorAll('[data-pg-next]').forEach(function(btn) {
    btn.addEventListener('click', function() {
...
EAP.wsjfWire = function(scope) {
  var root = scope || document;
  root.querySelectorAll('[data-wsjf-rank]').forEach(function(btn) {
...
EAP._findItemById = function(id) {
  ...
  ['Story','Defect','Incident','Problem'].forEach(function(k) { pools = pools.concat(bl[k] || []); });
...
EAP.showWsjfPopover = function(anchor, item, sticky) {
  ...
  var components = [
    { lbl: 'Business Value',   v: br.bv,   bg: '#2563EB' },
    { lbl: 'Time Criticality', v: br.tc,   bg: '#7C3AED' },
    { lbl: 'Risk Reduction',   v: br.rr,   bg: '#0891B2' },
    { lbl: 'Job Size',         v: br.size, bg: '#F97316' }
  ];
  ...
  pop.innerHTML =
    ...
    '<div class="wsjf-pop-legend">' +
      '<div class="wsjf-pop-legend-scale">' +
        '<span class="wsjf-pop-legend-item"><span class="wsjf wsjf-low">Later</span><span class="wsjf-pop-legend-rng">&lt;3</span></span>' +
        '<span class="wsjf-pop-legend-item"><span class="wsjf wsjf-mid">Plan</span><span class="wsjf-pop-legend-rng">3–10</span></span>' +
        '<span class="wsjf-pop-legend-item"><span class="wsjf wsjf-high">Do Now</span><span class="wsjf-pop-legend-rng">&gt;10</span></span>' +
      '</div>' +
    '</div>';
...
// ── Item name cell with contextual sub-label ──────────
EAP.itemNameCell = function(i) {
  var h = '<span class="item-nm">' + i.name + '</span>';
  var isBlocked = i.blocked || i.state === 'Blocked';
  if (isBlocked) {
    h += '<span class="item-sublbl item-sublbl-blocked">' + (i.blockReason || 'Blocked') + '</span>';
  } else if (i.atRisk) {
...
// ── Team pill with semantic colour tint ───────────────
EAP.teamPill = function(teamName) {
  if (!teamName || teamName === '—') return '<span class="tm">—</span>';
  var col = EAP.teamColors[teamName];
  if (!col) return '<span class="tm">' + teamName + '</span>';
  return '<span class="tm-pill"><span class="tm-dot" style="background:' + col + ';"></span>' + teamName + '</span>';
};
...
EAP.bkCols = function() {
  ...
  if (s.level === 'Epic') return {
    ...
    r: function(i) { return EAP._GT + '<td><span class="record-num">' + (i.num || '') + '</span></td><td>' + EAP.itemNameCell(i) + '</td><td>' + EAP.pill(i.state) + '</td><td><span class="par">' + (i.parent || '—') + '</span></td><td>' + EAP.ownerCell(i.owner) + '</td><td>' + EAP.wsjfCell(i) + '</td><td><span class="tm">' + (i.st || '—') + '</span></td><td>' + EAP.goalCell(i.goal) + '</td>'; }
  };
  if (s.level === 'Capability') return {
    ...
    r: function(i) { return EAP._GT + '<td><span class="record-num">' + (i.num || '') + '</span></td><td>' + EAP.itemNameCell(i) + '</td><td>' + EAP.pill(i.state) + '</td><td><span class="par">' + (i.parent || '—') + '</span></td><td>' + EAP.ownerCell(i.owner) + '</td><td>' + EAP.wsjfCell(i) + '</td><td><span class="tm">' + (i.art || '—') + '</span></td><td>' + EAP.goalCell(i.goal) + '</td>'; }
  };
  if (s.level === 'WorkItem') return {
    ...
    r: function(i) { return EAP._GT + '<td><span class="record-num">' + (i.num || '') + '</span></td><td>' + EAP.itemNameCell(i) + '</td><td>' + EAP.typeCell(i.type || 'Story') + '</td><td>' + EAP.pill(i.state) + '</td><td><span class="par">' + (i.parent || '—') + '</span></td><td>' + EAP.ownerCell(i.owner) + '</td><td><span class="sz">' + (i.pts || '') + '</span></td><td>' + EAP.teamPill(i.team || '—') + '</td><td>' + EAP.goalCell(i.goal) + '</td><td>' + EAP.readinessBadge(i) + '</td>'; }
  };
  // Feature
  return {
    ...
    r: function(i) { return EAP._GT + '<td><span class="record-num">' + (i.num || '') + '</span></td><td>' + EAP.itemNameCell(i) + '</td><td>' + EAP.pill(i.state) + '</td><td><span class="par">' + (i.parent || '—') + '</span></td><td>' + EAP.ownerCell(i.owner) + '</td><td>' + EAP.wsjfCell(i) + '</td><td>' + EAP.teamPill(i.team || '—') + '</td><td>' + EAP.goalCell(i.goal) + '</td><td>' + EAP.readinessBadge(i) + '</td>'; }
  };
};
...
EAP.lsCols = function() {
  ...
  if (s.level === 'Epic') return {
    ...
    r: function(i) { return EAP._GT + '<td><span class="record-num">' + (i.num || '') + '</span></td><td>' + EAP.itemNameCell(i) + '</td><td>' + EAP.pill(i.state) + '</td><td><span class="par">' + (i.parent || '—') + '</span></td><td>' + EAP.ownerCell(i.owner) + '</td><td>' + EAP.wsjfCell(i) + '</td><td><span class="tm">' + (i.st || '') + '</span></td><td>' + EAP.pbar(i.pct, i.state) + '</td><td>' + EAP.goalCell(i.goal) + '</td>'; }
  };
  if (s.level === 'Capability') return {
  ...
  return {
    ...
    r: function(i) { return EAP._GT + '<td><span class="record-num">' + (i.num || '') + '</span></td><td>' + EAP.itemNameCell(i) + '</td><td>' + EAP.pill(i.state) + '</td><td><span class="par">' + (i.parent || '—') + '</span></td><td>' + EAP.ownerCell(i.owner) + '</td><td>' + EAP.wsjfCell(i) + '</td><td>' + EAP.teamPill(i.team || '') + '</td><td>' + EAP.pbar(i.pct, i.state) + '</td><td>' + EAP.goalCell(i.goal) + '</td>'; }
  };
};
...
EAP._emptyStateSvg = '<svg width="96" height="96" viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg">' +
  ...
  '<line x1="60" y1="55" x2="60" y2="68" stroke="#D97706" stroke-width="3" stroke-linecap="round"/>' +
  '<circle cx="60" cy="74" r="2" fill="#D97706"/>' +
'</svg>';
