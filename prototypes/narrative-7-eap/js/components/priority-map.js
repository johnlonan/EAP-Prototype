...
EAP._pmItemInsight = function(item, quad) {
  ...
  var le = (item.size==='XS'||item.size==='S'||item.size==='M');
  if (!item.pi && w >= 9 && le)
    return 'Unscheduled Quick Win — commit to next PI before planning closes.';
  if (item.blocked)
    return 'Blocked: ' + (item.blockReason||'external dependency') + '. High value — unblocking this should be actioned this sprint.';
  if ((item.stalePIs||0) >= 3 && (quad==='timesink'||quad==='maybe'))
    return 'Passed over at ' + item.stalePIs + ' consecutive PI sessions with no team commitment — formal retirement or escalation to ART leadership is overdue.';
  if (item.atRisk && w >= 8)
    return 'At risk on a high-WSJF delivery — needs active mitigation before the end of this PI.';
  if (quad==='bigbet' && item.size==='XL')
    return 'XL-scope Big Bet. Consider a decomposition session to extract a schedulable Quick Win slice.';
  if (quad==='timesink') return 'Low value, high effort — defer or formally descope to protect capacity.';
  if (quad==='maybe')    return 'Low priority relative to current ART capacity. Defer unless a specific stakeholder dependency requires it.';
  if (quad==='quickwin' && !item.pi) return 'Quick Win with no PI commitment — schedule before the next planning cycle closes.';
  return 'No blockers or signals of concern detected.';
};
...
EAP._pmFindingsHTML = function(items) {
  return EAP._pmAIFindings(items).map(function(f, fi) {
    ...
    // Progressive disclosure — evidence (why) + action (recommended) collapse behind toggle
    var expandHtml = '';
    if (f.evidence || f.action) {
      var inner = '';
      if (f.evidence) inner +=
      ...
      expandHtml =
        '<button class="ins-sig-expand-toggle" data-expand="' + expId + '">Why this? ' + EAP.icon('chevron-down', 10) + '</button>' +
        '<div class="ins-sig-expand" id="' + expId + '">' + inner + '</div>';
    }
    ...
    return '<div class="ins-sig' + cls + ' ins-sig-ai" id="' + sigId + '">' +
      catHtml +
...
EAP._pmSyncHeader = function() {
  ...
  if (filterActive) {
    ...
    if (hdf) hdf.appendChild(pill);
  }
};

// ── Wire expand toggles within a container ────────────
EAP._pmWireExpand = function(container) {
  (container || document).querySelectorAll('.ins-sig-expand-toggle').forEach(function(btn) {
...
EAP._pmRefresh = function() {
  ...
  if (findingsEl) {
    findingsEl.innerHTML = EAP._pmFindingsHTML(items);
    EAP._pmWireExpand(findingsEl);
  }
...
EAP._pmDraw = function(items) {
  ...
  var qInterior = {
    quickwin:'rgba(22,163,74,0.04)',  bigbet:'rgba(180,83,9,0.04)',
...
EAP.openPriorityMap = function() {
  ...
  // Save ownerFilter and mineOnly so close can restore them without affecting backlog state.
  // mineOnly is suppressed while the PM is open — the filter popover uses ownerFilter only.
  EAP._pmPrevOwnerFilter = EAP.state.ownerFilter.slice();
  EAP._pmPrevMineOnly = EAP.state.mineOnly;
  EAP.state.mineOnly = false;
  // Only preset the owner filter for personas who auto-filter the backlog by ownership
  // (e.g. PM/Ananya). Exec, RTE, and dev personas open the strategic view unfiltered.
  var _pmPersona = EAP.personas[EAP.state.persona] || {};
  var _pmMineOnBacklog = (_pmPersona.mineOnTabs || []).indexOf('backlog') !== -1;
  if (!EAP.state.ownerFilter.length && EAP.ME && _pmMineOnBacklog) {
  ...
  // Wire expand toggles on initial render
  var findingsEl = el.querySelector('#pm-ai-findings');
  if (findingsEl) EAP._pmWireExpand(findingsEl);
...
EAP.closePriorityMap = function() {
  ...
  if (EAP._pmPrevMineOnly !== undefined) {
    EAP.state.mineOnly = EAP._pmPrevMineOnly;
    EAP._pmPrevMineOnly = undefined;
  }
  var tip = document.getElementById('pm-tip');
...
EAP._pmInjectStyles = function() {
  ...
  s.textContent = [
    ...
    /* Panel */
    '.pm-panel{width:92vw;height:90vh;background:#FFFFFF;',
      'border-radius:16px;',
    ...
    /* Expand toggle — visible on white panel background */
    '#pm-overlay .ins-sig-expand-toggle{color:#6B7280;}',
    '#pm-overlay .ins-sig-expand-toggle:hover{color:#374151;}',
    ...
    '#pm-overlay .fbar-filter-btn.active{background:rgba(56,55,51,0.10);border-color:rgba(56,55,51,0.22);}',
    '#pm-overlay .fbar-owner-pill{background:rgba(56,55,51,0.08);border-color:rgba(56,55,51,0.18);}',
    ...
    '.pm-tip-wsjf{color:#16A34A!important;background:rgba(22,163,74,0.09)!important;font-weight:600!important;}',
