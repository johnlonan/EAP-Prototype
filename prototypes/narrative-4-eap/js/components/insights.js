  ...
  if (s.tab === 'taskboard' && s.context === 'team' && s.trackMember && s.trackMember !== 'All') {
    ...
    if (mw) {
      ...
      h += '<div class="ins-member-predict">' +
        ...
        '<span class="ins-member-ppct" style="color:' + pColor + ';">' + pFinish + '% likely to finish sprint commitments</span>' +
        '</div></div>';
    }
  }
...
EAP.drawGauge = function(tv) {
  ...
  function draw(v) {
    ...
    x.strokeStyle = fc; x.lineWidth = 12; x.globalAlpha = 0.12; x.stroke();
    x.globalAlpha = 1;
    // Fill
