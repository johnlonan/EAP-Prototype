...
EAP.renderList = function() {
  ...
  if (s.splitView) {
    ...
    blHtml = '<div class="gpanel split-left"><div class="gpanel-hd"><div class="gpanel-hd-left"><span class="gpanel-title">Backlog</span><span class="gpanel-count">' + blD.length + '</span></div><button class="add-btn">' + EAP._ADD + 'New</button></div>' +
      ...
      blD.map(function(i) { return '<tr>' + bc.r(i) + '</tr>'; }).join('') +
      '</tbody></table></div></div>' +
  ...
  groups.forEach(function(g) {
    ...
    if (g.items && g.items.length) {
      acc += '<table class="dtbl"><thead><tr>' + cols.h + '</tr></thead><tbody>' +
        g.items.map(function(i) { return '<tr>' + cols.r(i) + '</tr>'; }).join('') + '</tbody></table>';
    } else {
  ...
  if (!s.splitView) {
    ...
    var bc2 = EAP.bkCols();
    acc += '<div class="pi-acc" style="margin-top:4px;"><div class="gpanel-hd" style="border-radius:16px 16px 0 0;"><div class="gpanel-hd-left"><span class="gpanel-title">Backlog</span><span class="gpanel-count">' + blD2.length + '</span></div><button class="add-btn">' + EAP._ADD + 'New</button></div><div style="padding:10px 14px 14px;"><table class="dtbl"><thead><tr>' + bc2.h + '</tr></thead><tbody>' + blD2.map(function(i) { return '<tr>' + bc2.r(i) + '</tr>'; }).join('') + '</tbody></table></div></div>';
  }
