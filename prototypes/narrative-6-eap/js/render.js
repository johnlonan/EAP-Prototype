...
EAP.renderFilterBar = function() {
  ...
  if (s.tab === 'taskboard') {
    ...
    h += '<div class="fbar-tv-wrap" id="track-view-dd">' +
      '<button class="fbar-vtog fbar-tv-trigger" id="track-view-trigger">' +
        tvIcon + '<span>' + tvLabel + '</span>' + EAP.icon('chevron-down', 10) +
      '</button>' +
  ...
  // Wire track view dropdown (Board | Workload)
  if (EAP._tvCloseHandler) { document.removeEventListener('click', EAP._tvCloseHandler); EAP._tvCloseHandler = null; }
  var tvTrigger = document.getElementById('track-view-trigger');
  var tvPanel = document.getElementById('track-view-panel');
  if (tvTrigger && tvPanel) {
    tvTrigger.addEventListener('click', function(e) {
      e.stopPropagation();
      tvPanel.style.display = tvPanel.style.display === 'block' ? 'none' : 'block';
    });
    tvPanel.querySelectorAll('[data-tv]').forEach(function(opt) {
      opt.addEventListener('click', function(e) {
        e.stopPropagation();
        EAP.state.trackView = opt.dataset.tv;
        EAP.render();
      });
    });
    EAP._tvCloseHandler = function(e) {
      var w = document.getElementById('track-view-dd');
      if (!w || !w.contains(e.target)) {
        var p = document.getElementById('track-view-panel');
        if (p) p.style.display = 'none';
      }
    };
    document.addEventListener('click', EAP._tvCloseHandler);
  }
