...
EAP.buildContextTree = function(query) {
  ...
  var s = EAP.state;
  var html = '';
  EAP._spacesExpanded = EAP._spacesExpanded || {};

  function matches(name) { return !ql || name.toLowerCase().indexOf(ql) !== -1; }

  function row(name, type, id, depth, isActive, spaceIds) {
    var indent = depth * 16;
    var typeLabels = { portfolio: 'Portfolio', 'solution-train': 'Sol. Train', art: 'ART', team: 'Team' };
    var treeLine = depth > 0
      ? '<span style="display:inline-flex;align-items:center;width:12px;flex-shrink:0;color:' + (isActive ? 'rgba(255,255,255,0.4)' : '#9CA3AF') + ';">└</span>'
      : '';

    // Spaces count badge — only for teams that have spaces, hidden during search
    var spacesBadge = '';
    ...
    return '<div class="ctx-row' + (isActive ? ' ctx-active' : '') + '" ' +
      'data-ctx-type="' + type + '" ' +
      'data-ctx-name="' + name.replace(/"/g, '&quot;') + '" ' +
      'data-ctx-id="' + id + '" ' +
      'style="padding:7px 12px 7px ' + (12 + indent) + 'px;display:flex;align-items:center;gap:6px;cursor:pointer;font-size:12px;transition:background 100ms ease;' + (isActive ? 'background:#343331;color:#fff;' : '') + '">' +
      treeLine +
      '<span style="font-size:9px;text-transform:uppercase;letter-spacing:0.04em;padding:1px 5px;border-radius:2px;flex-shrink:0;' +
        (isActive ? 'background:rgba(255,255,255,0.2);color:#fff;' : 'background:rgba(0,0,0,0.05);color:#6B7280;') + '">' +
  ...
  function spaceRow(spaceId, teamDepth) {
    var sp = EAP.spaces && EAP.spaces[spaceId];
    if (!sp) return '';
    var indent = (teamDepth + 1) * 16;
    var isActive = s.context === 'space' && s.contextId === spaceId;
    return '<div class="ctx-row ctx-space-row' + (isActive ? ' ctx-active' : '') + '" ' +
      'data-ctx-type="space" ' +
      'data-ctx-name="' + sp.name.replace(/"/g, '&quot;') + '" ' +
      'data-ctx-id="' + spaceId + '" ' +
      'style="padding:6px 12px 6px ' + (12 + indent) + 'px;display:flex;align-items:center;gap:6px;cursor:pointer;font-size:12px;transition:background 100ms ease;' + (isActive ? 'background:#343331;color:#fff;' : '') + '">' +
      '<span style="display:inline-flex;align-items:center;width:12px;flex-shrink:0;color:' + (isActive ? 'rgba(255,255,255,0.4)' : '#C7D2FE') + ';">└</span>' +
      '<span style="font-size:9px;text-transform:uppercase;letter-spacing:0.04em;padding:1px 5px;border-radius:2px;flex-shrink:0;' +
        (isActive ? 'background:rgba(255,255,255,0.2);color:#fff;' : 'background:rgba(99,102,241,0.10);color:#4338CA;border:1px solid rgba(99,102,241,0.18);') + '">Space</span>' +
      '<span style="flex:1;font-weight:' + (isActive ? '600' : '400') + ';color:' + (isActive ? '#fff' : '#374151') + ';">' + sp.name + '</span>' +
      '</div>';
  }

  var root = EAP.structure;
  // Was keyed on the now-retired kanbanView==='sprint' — the real intent
  // (restrict the picker to team depth for a team-scoped board) is better
  // expressed via context, which still holds true for James-style personas.
  var taskboardOnly = (s.tab === 'board' && s.context === 'team');
  ...
  // Solution Trains → ARTs → Teams → Spaces
  (root.children || []).forEach(function(st) {
    ...
    (st.children || []).forEach(function(art) {
      ...
      (art.children || []).forEach(function(team) {
        ...
        html += row(team.name, 'team', team.id, teamDepth, s.context === 'team' && s.contextId === team.id, team.spaces);
        // Render space children if this team is expanded and no search active
        if (team.spaces && team.spaces.length && EAP._spacesExpanded[team.id] && !ql) {
          team.spaces.forEach(function(spId) {
            html += spaceRow(spId, teamDepth);
          });
        }
      });
    });
...
EAP.wireContextRows = function() {
  // Spaces toggle badge — must stop propagation before row click fires
  document.querySelectorAll('.ctx-spaces-toggle').forEach(function(badge) {
    badge.addEventListener('click', function(e) {
      e.stopPropagation();
      var teamId = badge.dataset.teamId;
      EAP._spacesExpanded = EAP._spacesExpanded || {};
      EAP._spacesExpanded[teamId] = !EAP._spacesExpanded[teamId];
      // Re-render the tree in place, preserving search value
      var searchVal = '';
      var si = document.getElementById('ctx-search');
      if (si) searchVal = si.value || '';
      var tree = document.getElementById('ctx-tree');
      if (tree) { tree.innerHTML = EAP.buildContextTree(searchVal); EAP.wireContextRows(); }
    });
    badge.addEventListener('mouseenter', function() { badge.style.background = 'rgba(99,102,241,0.18)'; });
    badge.addEventListener('mouseleave', function() { badge.style.background = 'rgba(99,102,241,0.10)'; });
  });

  document.querySelectorAll('.ctx-row').forEach(function(row) {
    row.addEventListener('click', function(e) {
      // Let the spaces toggle handle its own click
      if (e.target.closest('.ctx-spaces-toggle')) return;
      e.stopPropagation();
...
EAP.applySearch = function(q) {
  ...
  if (tab === 'board') {
    // Search Kanban cards (PI view or sprint view)
...
// ── Board Drag & Drop ──────────────────────────────────
// The generic board (js/tabs/board.js) now owns the board tab entirely
// (all personas, all kanbanViews) via board-dnd.js. track.js's sprint/task
// board is retired — this legacy path is dead for the board tab and only
// still runs for any other .bcard producer that might exist elsewhere.
EAP.initBoardDrag = function() {
  if (EAP.state.tab === 'board') return;
  var content = document.getElementById('content-area');
  if (!content) return;
...
EAP.render = function() {
  ...
  setTimeout(function() {
    EAP.initBoardDrag();
    if (EAP.state.tab === 'board') {
      if (EAP.initBoardToolbar) EAP.initBoardToolbar();
      if (EAP.initGenericBoard) EAP.initGenericBoard();
      if (EAP.reapplyBoardPanelSelection && EAP.isBoardCardPanelOpen && EAP.isBoardCardPanelOpen()) EAP.reapplyBoardPanelSelection();
    }
