...
// ── Page header — breadcrumb + signpost title ───────────
// Both read live from EAP.tabConfig (state.js) by matching s.tab.
// Renaming a tab's label there is the only edit needed — this
// function has no hardcoded tab name to fall out of sync.
EAP.renderPageHeader = function() {
  var s = EAP.state;
  var el = document.getElementById('page-header');
  if (!el) return;

  var activeTab = EAP.tabConfig.filter(function(t) { return t.id === s.tab; })[0];
  var label = activeTab ? activeTab.label : s.tab;
  var homeHref = 'home.html' + (s.persona ? '?p=' + s.persona : '');

  /* Home glyph — same inline path already used for the nav's Home
     item (js/nav.js), reused here rather than a second icon system. */
  var homeIcon = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>';

  el.innerHTML =
    '<nav class="ph-breadcrumb" aria-label="Breadcrumb">' +
      homeIcon +
      '<a href="' + homeHref + '">Home</a>' +
      '<span class="ph-breadcrumb__sep">&rsaquo;</span>' +
      '<span>' + label + '</span>' +
    '</nav>' +
...
EAP.renderFilterBar = function() {
  ...
  if (s.tab === 'hierarchy') {
    ...
    // Owner filter (by avatar/owner) — hidden on the board tab, where
    // filtering now lives entirely in the board's own filter panel.
    var isGenericBoard = (s.tab === 'board');
    if (!isGenericBoard) {
      var ownerActive = (s.ownerFilter && s.ownerFilter.length > 0) || s.mineOnly;
      ...
      if (filterActive) {
        ...
        if (s.mineOnly && displayOwners.indexOf(EAP.ME) === -1) {
  ...
  // Dependencies dropdown — timeline only now. The board tab (all personas,
  // all kanbanViews) renders its own Dependencies toggle switch instead
  // (js/components/board-filters.js), on its own row alongside Compact.
  // track.js's sprint/task board is retired — the generic board handles it.
  if (s.tab === 'timeline' && (s.level === 'Feature' || s.level === 'WorkItem')) {
  ...
  // Compact toggle moved to the generic board's own row — see board-filters.js.
...
EAP.renderContent = function() {
  ...
  // Board tab: always the generic board now, for every persona/kanbanView.
  // track.js's sprint/task renderer is retired (unused, kept in the repo —
  // see project_narrative9_kanban_conform memory for why).
  ...
  el.addEventListener('click', function(e) {
    // Generic board cards (js/tabs/board.js) use their own editable
    // drawer — see js/components/board-panel.js. Never route them
    // through the shared read-only EAP.openDetail.
