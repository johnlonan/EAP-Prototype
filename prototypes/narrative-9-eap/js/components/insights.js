...
EAP.renderInsights = function() {
  ...
  var h = '<div class="ins-hdr">' +
    '<div class="ins-hdr-left"><span>Insights</span></div>' +
  ...
  if (d.gauge || d.predict) {
    ...
    var heroColor = heroVal >= 80 ? 'var(--dv-success)' : heroVal >= 65 ? 'var(--dv-warning)' : 'var(--dv-error)';
    var heroColorBg = heroVal >= 80 ? 'rgba(0,131,79,0.08)' : heroVal >= 65 ? 'rgba(141,110,0,0.08)' : 'rgba(220,38,38,0.08)';
    ...
    var solidHex = heroVal >= 80 ? '#00834F' : heroVal >= 65 ? '#f59e0b' : '#DC2626';
    var midHex   = heroVal >= 80 ? 'rgba(0,131,79,0.38)'   : heroVal >= 65 ? 'rgba(245,158,11,0.38)'   : 'rgba(220,38,38,0.38)';
    var upperHex = heroVal >= 80 ? 'rgba(0,131,79,0.13)'   : heroVal >= 65 ? 'rgba(245,158,11,0.13)'   : 'rgba(220,38,38,0.13)';
  ...
  if (d.flowDist) {
    var fd = d.flowDist;
    var fdSegments = [
      { pct: fd.features, label: 'Features', color: '#2563EB' },
      { pct: fd.defects, label: 'Defects', color: '#DC2626' },
      { pct: fd.enablers, label: 'Enablers', color: '#7C3AED' },
  ...
  // ── Inline sprint metrics — Kanban sprint view (WorkItem level) ──
  if (s.tab === 'board' && s.kanbanView === 'sprint' && s.level === 'WorkItem') {
  ...
  // ── Individual workload — Kanban sprint view when member is selected ──
  if (s.tab === 'board' && s.kanbanView === 'sprint' && s.context === 'team' && s.trackMember && s.trackMember !== 'All') {
  ...
  if (s.level === 'Feature' && (s.tab === 'planning' || s.tab === 'board') && s.context === 'art') {
    ...
    feats.forEach(function(f) {
      var col = f.state === 'Done' ? 'var(--dv-success)' : (f.blocked || f.state === 'Blocked') ? 'var(--dv-error)' : f.pct >= 40 ? 'var(--dv-primary)' : 'rgba(0,0,0,0.22)';
  ...
  if (d.signals) {
    ...
    if (urgent.length) {
      // Causal narrative: find the most common causeGroup across urgent signals
      var _cg = {}, _topCause = null, _topCount = 0;
      urgent.forEach(function(sig) { if (sig.causeGroup) _cg[sig.causeGroup] = (_cg[sig.causeGroup] || 0) + 1; });
      Object.keys(_cg).forEach(function(k) { if (_cg[k] > _topCount) { _topCause = k; _topCount = _cg[k]; } });
      var _causeLabels = { 'auth-api': 'Auth API contract unresolved — root cause of ' + _topCount + ' signals this sprint' };
      var _causeNote = (_topCause && _topCount >= 2)
        ? '<div class="ins-cause-note">' + EAP.icon('alert-triangle', 10) + '<span>' + (_causeLabels[_topCause] || _topCount + ' signals share a common root cause') + '</span></div>'
        : '';

      h += '<div class="ins-sig-group"><div class="ins-sec-lbl">Needs Attention <span class="ins-sig-count">' + urgent.length + '</span></div>' + _causeNote;
      urgent.forEach(function(sig, i) { h += renderSignal(sig, 'u' + i); });
      h += '</div>';
    }
    if (watch.length) {
      h += '<div class="ins-sig-group"><div class="ins-sec-lbl">Watching</div>';
      watch.forEach(function(sig, i) { h += renderSignal(sig, 'w' + i); });
      h += '</div>';
    }
    // One ok signal — quiet confirmation strip at the bottom, no card weight
    if (ok.length) {
      var _ok = ok[0];
      h += '<div class="ins-ok-strip">' +
        EAP.icon('check-circle', 12) +
        '<div class="ins-ok-body"><span class="ins-ok-title">' + _ok.title + '</span><span class="ins-ok-desc">' + _ok.desc + '</span></div>' +
        '</div>';
    }
  }
  ...
  el.querySelectorAll('.ins-sig-expand-toggle').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      e.stopPropagation();
      var expandEl = document.getElementById(btn.getAttribute('data-expand'));
      if (!expandEl) return;
      var isOpen = expandEl.classList.toggle('open');
  ...
  // Cursor affordance — click handling wired via document capture in interactions-pass.js
...
function renderMiniChart(mc) {
  ...
  if (mc.type === 'hbars') {
    ...
    if (mc.wipLimit !== undefined) {
      ...
      s += '<line x1="'+limX+'" y1="0" x2="'+limX+'" y2="'+VH2+'" stroke="rgba(220,38,38,0.25)" stroke-width="1" stroke-dasharray="3,2"/>';
      s += '<text x="'+(limX+2)+'" y="8" font-size="7" fill="rgba(220,38,38,0.55)" font-family="var(--font-sans)">WIP limit: '+mc.wipLimit+'</text>';
    }
  ...
  if (mc.type === 'bars') {
    ...
    bVals.forEach(function(v, i) {
      ...
      var by3 = pT3 + cH3 - bH3;
      var col = mc.colors ? mc.colors[i] : '#2563EB';
  ...
  if (mc.type === 'forecast') {
    var VH4 = 52;
    var fCol = mc.median >= 80 ? '#00834F' : mc.median >= 60 ? '#f59e0b' : '#DC2626';
...
function renderSignal(s, id) {
  ...
  // Actions row — aging badge sits left of the action buttons
  var agingHtml = '';
  if (s.agingDays) {
    var agingColor = s.agingDays >= 7 ? 'var(--dv-error)' : 'var(--dv-warning)';
  ...
  // Progressive disclosure — why + recommended collapse behind "Why this?" toggle
  if (s.why || s.recommended) {
    h += '<button class="ins-sig-expand-toggle" data-expand="' + expandId + '">Why this? ' + EAP.icon('chevron-down', 10) + '</button>';
    h += '<div class="ins-sig-expand" id="' + expandId + '">';
...
EAP.drawMCTimeline = function() {
  ...
  svg.append('line')
    ...
    .attr('y1', lineY).attr('y2', lineY)
    .attr('stroke', 'rgba(0,0,0,0.15)').attr('stroke-width', 7).attr('stroke-linecap', 'round');
  ...
  var markers = [
    ...
    { days: mc.p85days, color: '#383733',           r: 5.5, tip: '85th pct — conservative: ' + mc.p85 }
  ];
...
EAP.wireInsightActions = function() {
  document.querySelectorAll('[data-ins-action]').forEach(function(link) {
    link.addEventListener('click', function(e) {
      ...
      // Mark parent signal card as actioned
      var card = link.closest('.ins-sig');
      if (card) card.classList.add('ins-sig-actioned');
      ...
      else if (action === 'Plan into PI 27') { EAP.openDetail('f7'); EAP.showToast('Opened for planning', 'info'); }
...
EAP.drawHealthHeatmap = function() {
  ...
  var sc = {
    ...
    over:    { bg: 'rgba(220,38,38,0.09)',  text: '#DC2626', hover: 'rgba(220,38,38,0.18)' }
  };
...
EAP.drawPanelBurndown = function(pc) {
  ...
  var pri = '#2563EB', warn = '#f59e0b', idealCol = 'rgba(0,0,0,0.18)';
