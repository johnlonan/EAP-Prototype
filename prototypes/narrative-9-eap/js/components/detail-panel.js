...
var DP_DESC = {
  ...
  Incident: 'This incident represents an unplanned interruption or degradation of service identified in testing or production. It is triaged, prioritised, and resolved within the current sprint or backlog.',
  Problem: 'This problem represents the underlying root cause of one or more incidents or defects. It is under investigation to identify a permanent fix and prevent recurrence across the system.'
};
...
function dpRender(item) {
  ...
  var dpIconKey = type.toLowerCase() === 'incident' ? 'alert-circle' : type.toLowerCase() === 'problem' ? 'layers' : type.toLowerCase();
  h += '<div class="dp-type" style="color:' + tc + ';">' + EAP.icon(dpIconKey, 14) + ' ' + type + '</div>';
  ...
  var fields = [];
  if (item.owner) fields.push({ label: type === 'Story' || type === 'Defect' || type === 'Incident' || type === 'Problem' ? 'Assigned to' : 'Owner', value: '<span class="dp-owner">' + EAP.avatar(item.owner, 20) + ' ' + (EAP.people[item.owner] ? EAP.people[item.owner].name : item.owner) + '</span>' });
